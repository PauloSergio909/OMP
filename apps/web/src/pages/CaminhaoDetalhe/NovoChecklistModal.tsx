...
import { useItensPadraoChecklist, useCriarChecklist, useMotoristasDisponiveis } from '../../hooks/useApi';
...
interface Props {
  ...
  caminhaoId: string;
  kmAtual: number;
  motoristaPadrao?: { id: string; nome: string } | null;
}

export function NovoChecklistModal({ open, onClose, caminhaoId, kmAtual, motoristaPadrao }: Props) {
  const { data: itensPadrao = [] } = useItensPadraoChecklist();
  const { data: motoristasDisponiveis = [] } = useMotoristasDisponiveis();
  ...
  useEffect(() => {
    if (open) {
      setTipo('pre_viagem');
      setKm(String(kmAtual));
      setMotoristaId(motoristaPadrao?.id ?? '');
      setObservacoes('');
  ...
  return (
    <Modal open={open} onClose={onClose} title="Novo Checklist de Vistoria" size="lg">
      <div className="space-y-4">
        <div className="grid grid-cols-3 gap-4">
          ...
          <Field label="Responsável" required>
            <select className={selectCls} value={motoristaId} onChange={(e) => setMotoristaId(e.target.value)}>
              <option value="">Selecione...</option>
              {motoristaPadrao && (
                <option value={motoristaPadrao.id}>{motoristaPadrao.nome}</option>
              )}
      ...
      <ModalFooter
        ...
        disabled={!motoristaId || !km}
        confirmLabel="Salvar Checklist"
      />
    </Modal>
  );
}
